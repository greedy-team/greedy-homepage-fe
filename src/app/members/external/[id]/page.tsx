...
import { formatStackPosition, getAvatarUrl } from "@/entities/member/lib";
...
/**
 * 든든한 리뷰어(외부) 프로필. 백엔드에 이 사람들의 자기소개·프로젝트 이력을 저장할 곳이 없어서
...
export default async function ExternalReviewerProfilePage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-10 px-5 py-16 md:px-20 md:py-20">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant="brand">{formatStackPosition(reviewer.stackPosition)}</Badge>
      ...
      {reviewer.reviewedGenerations.length > 0 && (
        <section className="flex flex-col gap-4">
          <h2 className="text-h3 text-text">{PROFILE.historyTitle}</h2>
          <ul className="flex flex-col gap-3">
            {reviewer.reviewedGenerations.map((generationNumber) => (
              <li key={generationNumber}>
                <Card className="flex items-center gap-3 p-4">
                  <Badge variant="brand">{generationNumber}기</Badge>
                  <span className="text-body text-text">리뷰어</span>
                </Card>
              </li>
            ))}
          </ul>
