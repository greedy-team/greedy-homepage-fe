...
| `ActivityType` enum | 회장·영입리드 없음 (⚠️로 표시만) | **현행 5개 값 유지로 확정** | 실데이터 전수 조사 결과 화면 역할이 전부 기존 enum과 1:1 대응(운영진=MAINTAINER 포함, 1기까지 확인). 유일한 예외 "회장"은 enum에 추가하지 않고 프론트 큐레이션으로 유지하기로 확정 — §5-8. |
