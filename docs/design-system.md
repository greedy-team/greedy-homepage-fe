...
- 숨기고 보이는 건 감싸는 요소로 제어해요. 컴포넌트에 `className="hidden"`을 줘도 숨겨지지 않아요. `cn`은 클래스를 이어 붙이기만 해서 컴포넌트의 기본 `inline-flex`와 덧붙인 `hidden`이 둘 다 남고, 이때는 CSS 파일에 나중에 적힌 `inline-flex`가 적용돼요. 에러가 나지 않으니 그냥 지나치기 쉬워요.
