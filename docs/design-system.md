...
- 누르는 것의 최소 높이는 44px이에요. sm 버튼은 모바일에서 md로 키워요. 겉모습을 못 키우는 작은 요소(칩 등)는 가상 요소로 눌리는 영역만 넓혀요.
- 누르는 것에는 공통 포커스 링을 붙여요. `src/lib/cn.ts`의 `focusRing`이에요. 키보드로 쓰는 사람이 지금 어디에 있는지 봐야 해요.
- 캐러셀은 자동 재생하지 않아요.
...
| TimelineItem | date, last | 세로 타임라인 한 칸 |
