...
- 본문 문단은 폭을 제한하지 않아요. 줄바꿈은 콘텐츠가 문장 단위로 정해요 (`\n` + `whitespace-pre-line`). 화면 폭의 자동 줄바꿈에 기대지 않고 문장 끝에서 끊어요.
...
| BottomCta | href, label, caption | 하단 고정 버튼. 모집 중 랜딩에서 히어로를 지나면 나타나요. 좁은 화면 전용(md:hidden)과 노출 시점은 쓰는 곳에서 정해요 |
